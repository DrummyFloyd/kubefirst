...
var (
	...
	// Supported providers
	supportedDNSProviders = []string{"cloudflare", "ovh"}
...
func NewCommand() *cobra.Command {
	ovhCmd := &cobra.Command{
		Use:   "ovh",
		Short: "kubefirst ovh installation",
		Long:  "kubefirst ovh",
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Println("To learn more about ovh in kubefirst, run:")
			fmt.Println("  kubefirst ovh --help")
	...
	ovhCmd.AddCommand(Create(), Destroy(), RootCredentials())
...
func Create() *cobra.Command {
	createCmd := &cobra.Command{
		Use:              "create",
		Short:            "create the kubefirst platform running on ovh kubernetes",
		TraverseChildren: true,
		RunE:             createOvh,
	}

	ovhDefaults := constants.GetCloudDefaults().Ovh
	...
	createCmd.Flags().StringVar(&cloudRegionFlag, "cloud-region", "gra9", "the ovh region to provision infrastructure in")
	createCmd.Flags().StringVar(&clusterNameFlag, "cluster-name", "kubefirst", "the name of the cluster to create")
	createCmd.Flags().StringVar(&clusterTypeFlag, "cluster-type", "mgmt", "the type of cluster to create (i.e. mgmt|workload)")
	createCmd.Flags().StringVar(&nodeCountFlag, "node-count", ovhDefaults.NodeCount, "the node count for the cluster")
	createCmd.Flags().StringVar(&nodeTypeFlag, "node-type", ovhDefaults.InstanceSize, "the instance size of the cluster to create")
...
func Destroy() *cobra.Command {
	destroyCmd := &cobra.Command{
		...
		Long:  "destroy the kubefirst platform running in ovh and remove all resources",
		RunE:  common.Destroy,
